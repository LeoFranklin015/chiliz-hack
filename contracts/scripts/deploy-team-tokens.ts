...
async function mintTokensToAccount(targetAccount: string, amountPerToken: string = "1000000") {
  console.log(`\n🪙 Minting ${amountPerToken} tokens to account: ${targetAccount}`);
  
  try {
    // Read the deployed token addresses
    const tokenAddresses = JSON.parse(fs.readFileSync('teamFanTokenAddress.json', 'utf8'));
    ...
    for (const [symbol, address] of Object.entries(tokenAddresses)) {
      try {
        ...
        const FanToken = await ethers.getContractAt("FanToken", address as string);
        const mintTx = await FanToken.mint(targetAccount, mintAmount);
        await mintTx.wait();
        
        console.log(`✅ Minted ${amountPerToken} ${symbol} tokens to ${targetAccount}`);
...
mintTokensToAccount("0xCD13f3b0B929b4E0A7B0daeEca81F6ae9F21f2CA", "5000000").catch((error) => {
  console.error(error);
  process.exitCode = 1;
});
